...
class OcrEngine private constructor(
  ...
  companion object {
    ...
    // Must match the dictionary the recognition model was trained with: the CTC head has
    // exactly (dictionary entries + 1 blank) classes. Swapping the model without swapping
    // this file yields garbage text rather than an error.
    private const val KEYS_FILE = "keys_eslav.txt"
    ...
    private fun copyAssetToCache(context: Context, assetPath: String): String {
      ...
      // Re-extract whenever the APK is newer than the cached copy. Assets are replaced on
      // every install/update, so a model swap that keeps the same file name would otherwise
      // keep running from a stale cache entry.
      val installedAt = context.packageManager
        .getPackageInfo(context.packageName, 0)
        .lastUpdateTime

      if (!cacheFile.exists() || cacheFile.lastModified() < installedAt) {
