    ...
    public:
        ...
        bool Initialize(const std::string &model_path,
            ...
            if (output_type_result) {
                ...
                // The CTC head has one blank plus one class per dictionary entry. When the
                // counts disagree the model and the dictionary were built for different
                // languages, and CtcDecode maps indices onto the wrong characters without
                // failing, so surface it here.
                const int expected_classes = static_cast<int>(dictionary_.size()) + 1;
                if (num_classes_ != expected_classes) {
                    LOGE(TAG,
                         "Dictionary/model mismatch: %zu dictionary entries imply %d classes, "
                         "but the model outputs %d. Recognized text will be wrong.",
                         dictionary_.size(), expected_classes, num_classes_);
                }
            }
